...
shared class TDMSpawns : RespawnSystem
{
	...
	{
		//default - in queue spawn
		u8 spawn_property = 254;

		// i think this is suppose to be delayless spawn or something
		// i dont speak machine languange unlike v1
	...
	void DoSpawnPlayer(PlayerInfo@ p_info)
	{
		if (force || canSpawnPlayer(p_info))
		{
			...
			// team
			if (player.getTeamNum() != int(p_info.team)){player.server_setTeamNum(p_info.team);}
			...
			// reset the previous blobinfo if its not builder class
			// this is to prevent YOU becoming knight or something

			string spawnBlob = p_info.blob_name;
			if (spawnBlob != "builder" && spawnBlob != "enginer" && spawnBlob != "trapmaster") {
				spawnBlob = "builder";
			}

			// manually spawn blob
			CBlob @playerBlob = server_CreateBlob(spawnBlob, p_info.team, getSpawnLocation(p_info));
			...
			{
				// spawn resources , i don t know what does it do but whatev
				p_info.spawnsCount++;
...
shared class TDMCore : RulesCore
{
	...
	{
		TDMPlayerInfo p(player.getUsername(), player.getTeamNum(), "builder");
		players.push_back(p);
	...
	void CheckTeamWon()
	{
		// no winning when game aint even started buddy
		...
		if (sudden_death)
		{
			...
			{
				// can only win if the current boss is defeated
				if (rules.exists("bossNetID")) {
					u16 bossNetID = rules.get_netid("bossNetID");
					if (bossNetID > 0) {
