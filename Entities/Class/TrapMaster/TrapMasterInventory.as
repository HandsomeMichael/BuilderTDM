...
#include "Help.as";
#include "CommonTrapMasterBlocks.as";
...
namespace Builder
{
	enum Cmd
	{
		...
		make_reserved = 99,
		unequip_armor = 100 
		// maybe instead of making enums. we actually add commandId ??
	};
...
void MakeBlocksMenu(CInventory@ this, const Vec2f &in INVENTORY_CE)
{
	...
	if (menu !is null)
	{
		...
		const Vec2f ARMOR_POS = (menu.getUpperLeftPosition() - Vec2f(GRID_PADDING, 0) + Vec2f(-1, 1) * GRID_SIZE / 2) - Vec2f(0,20);
		...
		if (blob.hasTag("armored")) {
			CGridMenu@ armor = CreateGridMenu(ARMOR_POS, blob, Vec2f(1, 1), "");
			if (armor !is null)
			{
				armor.SetCaptionEnabled(false);

				CBitStream params;
				params.write_u16(blob.getNetworkID());

				CGridButton@ armorbutton = tool.AddButton("$chainarmor$", "", Builder::unequip_armor, Vec2f(1, 1), params);
				if (armorbutton !is null)
				{
					armorbutton.SetHoverText(getTranslatedString("Unequip armor\n"));
				}
			}
		}
	}
...
void onCommand(CInventory@ this, u8 cmd, CBitStream@ params)
{
	...
	else if (cmd == Builder::unequip_armor)
	{
		u16 id;
		if (!params.saferead_u16(id)) return;
