// Patch : Attempted to add custom materials quantity
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	if (cmd == this.getCommandID("shop made item"))
	{
		...
		string name = params.read_string();
		{
			...
			{
				CBlob@ mat = server_CreateBlob(split[0]);
				if (mat !is null){
					mat.Tag('custom quantity');
					mat.server_SetQuantity(parseInt(split[1]));
					if (!callerBlob.server_PutInInventory(mat)){mat.setPosition(callerBlob.getPosition());}
				}
			}
		}
	}
