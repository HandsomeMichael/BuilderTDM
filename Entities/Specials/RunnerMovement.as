...
void onTick(CMovement@ this)
{
	...
	if ( control !is null)
	{
		...
		{
			// dodge direction is overrided by key control
			bool dodgeDirectionLeft = blob.isFacingLeft();
			if (left) {dodgeDirectionLeft = true;}
			if (right) {dodgeDirectionLeft = false;}

			blob.Tag("dodgeroll");
			blob.set_u32("dodgeroll_time",getGameTime() + 60);

			// 10 tick of immunity , apparently kag has a funny immune time system 
			...
			if (blob.isOnGround() && !blob.isInWater()) {
				blob.AddForce(Vec2f((dodgeDirectionLeft ? -200.0f : 200.0f ),0));
			}
			else {
				blob.AddForce(Vec2f((dodgeDirectionLeft ? -100.0f : 100.0f ),0));
			}

			blob.getSprite().PlaySound("/Stun.ogg");
			return;

			//CBitStream params;
			//this.SendCommand(this.getCommandID("dodgeroll"), params);
		}
	}
...
// but how will this effect the industrial society and its concequences ?!!??!!?
