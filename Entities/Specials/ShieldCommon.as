...
bool otherBlockAttack(CBlob@ this, Vec2f direction, f32 damage) 
{
	// check if player carrying a shield
	CBlob@ shield = this.getCarriedBlob();
	if (shield is null) return false;
	if (shield.getName() != "smollshield") return false;

	// instead of doing wacky rotation check , i just did a lazy facing check
	// i am gud programmer
	bool hitTheAttack = (direction.x > 0 && !shield.isFacingLeft()) || (direction.x < 0 && shield.isFacingLeft());
	return !hitTheAttack;
}
bool blockAttack(CBlob@ this, Vec2f direction, f32 damage)
{
	...
	ShieldVars@ vars = getShieldVars(this);
	if (vars is null) { 
		return otherBlockAttack(this,direction,damage); 
	}
