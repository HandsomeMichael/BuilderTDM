// Patch : remove water requirement for crates and ruins restock, i hate kag
...
void GetButtonsFor(CBlob@ this, CBlob@ caller)
{
	...
	else if (this.hasTag("unpackall") && caller.getTeamNum() == this.getTeamNum()) 
	{
		// only teammates can open up crate , but enemies can still stole it and break the crat

		// tell wich player has unpacked the restock
		CBitStream params;
		params.write_netid(caller.getNetworkID());
		
		caller.CreateGenericButton(12, buttonpos, this, this.getCommandID("unpack"), getTranslatedString("Unpack all"),params);
	}
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	if (cmd == this.getCommandID("unpack"))
	{
		...
		{
			// only used on one occasion , i might make a restock blob instead of spawning crates
			CBlob@ unpacker = getBlobByNetworkID(params.read_netid());

			// send message to all teammates on client
			if (unpacker !is null && isClient()) 
			{
				// unpacker player
				CPlayer@ player = unpacker.getPlayer();

				// check if ruinsID is valid
				if (player !is null && this.get_u16("ruinsID") > 0) 
				{
					// check if crate and client player is the same team
					if (getLocalPlayer().getTeamNum() == this.getTeamNum()) 
					{
						// send message
						client_AddToChat(player.getUsername()+" has taken the restock crate");
					}
				}
			}
...
void onDie(CBlob@ this)
{
	...
	u16 ruinsID = this.get_u16("ruinsID");
	if (ruinsID > 0) {
		CBlob@ ruins = getBlobByNetworkID(ruinsID);
		if (ruins !is null) {
			ruins.set_u32("drop_mats",getGameTime()+1500);
			ruins.Untag("wait");
		}
	}
}
...
bool canUnpackHere(CBlob@ this)
{
	...
	// fuck it , you can unpack anywhere . hehehehaw
