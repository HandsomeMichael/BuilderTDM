// Patch : added something fum on enginer gaming
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
	...
	if (this.getPosition().y < blob.getPosition().y - 2)
	{
		...
		// enginer gaming
		if (blob.getName() == "enginer") 
		{
			if (blob.getSprite().isAnimation("strike")) 
			{
				enemydam += 1.5f;
			}
		}
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	if (customData == Hitters::stomp && damage > 0.0f && velocity.y > 0.0f && worldPoint.y < this.getPosition().y)
	{
		...
		// enginer immune to stomp stun
		if (this.getName() != "enginer") {
